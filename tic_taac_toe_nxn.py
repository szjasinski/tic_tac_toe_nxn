...
def my_click(button_number):
    ...
    # for x in this_list:
    #     num = this_list.index(x)+1
    #     exec("if button_number == " + str(num) + ":")
    #     exec("  button_" + str(num) + ' = Button(my_frame, text=get_sign(), font=("Helvetica", 32), padx=55, pady=50)')
    #     eval("  button_" + str(num) + ".grid(row=2, column=2))
    #     win_check(button_number)

    for x in this_list:
        num = this_list.index(x)+1
        my_string = """    if button_number == :
            button_9 = Button(my_frame, text=get_sign(), font=("Helvetica", 32), padx=55, pady=50)
            button_9.grid(row=2, column=2)
            win_check(button_number)
            """
...
n = 5
this_list = ["button_" + str(x) for x in range(1, n*n+1)]

for x in this_list:
    exec(x + " = Button(my_frame, text=" + '"  "' + ", font=(" + '"Helvetica"' + ", 32), padx=55, pady=50, command=lambda: my_click(" + str(this_list.index(x)+1) + "))")


counter = 0
for y in range(0, n):
    for z in range(0, n):
        eval(this_list[counter] + ".grid(row=" + str(y) + ", column=" + str(z) + ")")
        counter += 1
